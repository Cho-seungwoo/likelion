...
class IdentitiesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create identity" do
    assert_difference('Identity.count') do
      post identities_url, params: { identity: { content: @identity.content, title: @identity.title } }
    end
  ...
  test "should update identity" do
    patch identity_url(@identity), params: { identity: { content: @identity.content, title: @identity.title } }
